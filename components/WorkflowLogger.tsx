...
export function WorkflowLogger({ events, status, runId }: WorkflowLoggerProps) {
  ...
  const getLabel = (event: WorkflowEvent) => {
    ...
    switch (event.eventType) {
      case "step_started":
        return `Started step: ${event.correlationId} (Attempt ${
          payload.attempt || 0
        })`;
      case "step_completed":
        return `Completed step: ${event.correlationId}`;
      case "step_failed":
        return `Failed step: ${event.correlationId} - ${payload.error}`;
      case "step_retrying":
        return `Retrying step: ${event.correlationId} (Attempt ${payload.attempt})`;
      case "wait_created":
        return `Sleeping for ${payload.duration}ms...`;
      case "wait_completed":
        return `Woke up from sleep`;
      case "signal_waiting":
        return `Waiting for signal: ${payload.name}`;
      case "signal_received":
        return `Received signal: ${
          event.correlationId?.replace("signal-", "")
        }`;
      case "rollback_registered":
        return `Registered rollback: ${payload.method}`;
      default:
      ...
      <div className="card-body p-0 flex flex-col h-full">
        <div className="p-4 border-b border-base-content/10 flex-none flex justify-between items-center">
          ...
          <div className="flex items-center gap-2">
            <span className="badge badge-ghost badge-sm border-base-content/10">
            ...
            {runId && (
              <span className="text-[10px] opacity-20 font-mono hidden sm:inline">
                ID: {runId}
              </span>
            )}
          </div>
            ...
            >
              ...
              <div className="flex-1">
                <div className="flex justify-between">
                  <span
                    className={event.eventType.includes("failed")
                      ? "text-error"
                      : ""}
                  >
                    {getLabel(event)}

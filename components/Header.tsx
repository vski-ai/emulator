...
import { Settings, Database, Cloud, HelpCircle, Github, Info, Compass } from "lucide-react";
  ...
  return (
    <header className="navbar bg-base-100 border-b border-base-content/10 px-4 sticky top-0 z-50 backdrop-blur-md bg-opacity-80">
      <div className="flex-1 flex items-center gap-4">
        <a href="/" className="btn btn-ghost text-2xl gap-2 font-black tracking-tighter italic p-0 hover:bg-transparent">
      ...
      <div className="flex-none flex items-center gap-4">
        ...
        <div className={`badge ${
          isReady ? 'badge-success' : (mode === 'url' && apiUrl ? 'badge-error' : 'badge-warning')
        } badge-sm font-bold tracking-widest text-[9px] ${!isReady && mode === 'url' ? 'animate-pulse' : ''}`}>
          {isReady ? 'READY' : (mode === 'url' && apiUrl ? 'ERROR' : 'CONNECTING')}
        </div>
      </div>
        ...
        <div className="modal modal-open">
          <div className="modal-box max-w-2xl border-2 border-primary/20 shadow-2xl">
            <div className="flex items-start gap-4 mb-6">
              ...
              <div>
                <h3 className="text-2xl font-black italic tracking-tight uppercase">VSKI Durable Execution</h3>
                <p className="text-sm opacity-50 font-bold uppercase tracking-widest">Infrastructure in your pocket</p>
              </div>
            </div>
            ...
            <div className="space-y-4 text-sm leading-relaxed overflow-y-auto max-h-[60vh] pr-2">
              ...
              <div className="bg-base-200 p-4 rounded-xl space-y-2 border border-base-content/5">
                ...
                <ul className="list-disc list-inside opacity-70 space-y-1 text-xs">
                  ...
                  <li>"Code as Infrastructure" - no more complex state machines in DB</li>
                </ul>
              </div>
            ...
            <div className="modal-action">
              <button className="btn btn-primary btn-sm font-black uppercase tracking-widest" onClick={() => setShowAbout(false)}>
                Understood
              </button>
            </div>
          </div>
          <form method="dialog" className="modal-backdrop" onClick={() => setShowAbout(false)}>
            <button>close</button>
          </form>
        </div>,
        document.body

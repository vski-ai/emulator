...
export const WORKFLOW_DEMOS: WorkflowDemo[] = [
  {
    ...
    actors: [
      ...
      { name: "Manager", role: "manager", signals: [
        { name: "manager-approval", label: "Approve", data: { approved: true } },
        { name: "manager-approval", label: "Reject", data: { approved: false } }
      ]}
    ]
  },
  {
    ...
    actors: [
      ...
      { name: "New User", role: "user", signals: [
  ...
  },
  {
    ...
    actors: [
      ...
      { name: "Warehouse", role: "manager", signals: [
  ...
  },
  {
    ...
    name: "IoT Escalation",
    category: "IoT",
    description: "Handle device alerts with automatic escalation if not acknowledged.",
    icon: "Radio",
    color: "warning",
    defaultInput: ["sensor_kitchen_01"],
    actors: [
      { name: "Device", role: "system" },
      { name: "Technician", role: "user", signals: [
  ...
  },
  {
    ...
    actors: [
      ...
      { name: "User", role: "user", signals: [
        { name: "reset-password", label: "Submit New Password", data: { pass: "secret123" } }
      ]}
    ]
  },
  {
    ...
    actors: [
      { name: "Dispatcher", role: "system", signals: [
        { name: "agent-assignment", label: "Assign Agent", data: { agent: "Bob" } }
      ]},
      { name: "Support Agent", role: "manager", signals: [
  ...
  },
  {
    ...
    category: "Logistics",
    isTop: true,
    description: "Multi-warehouse complectation with manager review and recursive retry loops.",
    icon: "Truck",
    color: "primary",
    ...
    hasFlowchart: true,
    actors: [
      { name: "WH Manager A", role: "manager", signals: [
        { name: "warehouse-a-pick", label: "Pack Item A", data: {} }
      ]},
      { name: "WH Manager B", role: "manager", signals: [
        { name: "warehouse-b-pick", label: "Pack Item B", data: {} }
      ]},
      { name: "Delivery Admin", role: "manager", signals: [
        { name: "manager-shipment-review", label: "Approve Shipment", data: { action: "approve" } },
        { name: "manager-shipment-review", label: "Reject A (Defect)", data: { action: "reject-a" } },
        { name: "manager-shipment-review", label: "Reject B (Defect)", data: { action: "reject-b" } },
        { name: "manager-resolution", label: "Trigger Refund", data: { action: "refund" } },
        { name: "manager-resolution", label: "Fix and Re-ship", data: { action: "re-complectate" } }
      ]},
      { name: "Delivery Driver", role: "user", signals: [
        { name: "driver-pickup", label: "Pick Up Package", data: {} },
        { name: "delivery-attempt-result", label: "Deliver Success", data: { status: "success" } },
        { name: "delivery-attempt-result", label: "Fail (Weather)", data: { status: "fail" } }
      ]},
      { name: "Customer", role: "user", signals: [
        { name: "client-decision", label: "Accept Order", data: { action: "accept" } },
        { name: "client-decision", label: "Reject (Damaged)", data: { action: "reject" } }
      ]}
    ]
  },
  {
    ...
    category: "AI",
    isTop: true,
    description: "Automated safety checks with human escalation and parallel AI translations.",
    icon: "Cpu",
    color: "secondary",
    defaultInput: ["My new blog post about RocketBase..."],
    actors: [
      { name: "AI Agent", role: "system", signals: [
        { name: "ai-safety-check", label: "AI: Looks Safe", data: { safe: true, escalate: false } },
        { name: "ai-safety-check", label: "AI: Escalate", data: { safe: true, escalate: true } }
      ]},
      { name: "Lead Moderator", role: "manager", signals: [
        { name: "mod-1-approve", label: "Mod 1: Approve", data: {} }
      ]},
      { name: "Junior Moderator", role: "manager", signals: [
        { name: "mod-2-approve", label: "Mod 2: Approve", data: {} }
      ]},
      { name: "Proofreader", role: "user", signals: [
